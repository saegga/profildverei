...
else
{
	if(!empty($_REQUEST["add_item"]))
	{
		...
		{
			Diag\Debug::writeToFile($_REQUEST, $varName = "reguest", $fileName = "debug.txt");
			...
			else
			{
				...
				{
					/*nabor*/
					if(!empty($_REQUEST['naborIds'])){
						$arrNabor = $_REQUEST['naborIds'];

						foreach($arrNabor as $nabor){
							$product_properties = true;
							if (!empty($_REQUEST["naborOffersCartProps"]))
							{
								$product_properties = CIBlockPriceTools::GetOfferProperties(
									$nabor['idNabor'],
									$_REQUEST["naborIblock"],
									$_REQUEST["naborOffersCartProps"]
								);
							}

							if($nabor['idNabor']){
								$ratio = $nabor['ratio'] ? $nabor['ratio'] : 1;
								Add2BasketByProductID($nabor['idNabor'], $ratio, ['LID' => $_REQUEST['naborLid']], $product_properties);
								if ($ex = $APPLICATION->GetException()){
									$strError = "ERROR_ADD2BASKET";
									$successfulAdd = false;
								}
