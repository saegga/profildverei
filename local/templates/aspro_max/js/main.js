	...
	function initSelects(target){
		...
		$(target).find('.wrapper1 select:visible').ikSelect({
			...
			onShow: function(inst){
				...
				// for custom element_6.php
				if(inst.$dropdown[0].classList.contains("element_6_listdropdown")){

					var scrollDiv = inst.$listInner;
					
					scrollDiv.css('height', "auto");
					scrollDiv.css('max-height', "250px");
				}
			},
...
$(document).ready(function(){
	...
	$(document).on("click", ".counter_block:not(.basket) .plus", function(){
		...
		var block = $(".prices_block").clone(true);
	...
	BX.addCustomEvent('onCounterProductAction', function(eventdata) {
		...
		try{
			...
			/**/
			// var block = eventdata.params.id.closest('.main_item_wrapper');
			// window[obProduct].changePriceCustom2(block);
		}
		catch(e){
			console.error(e)
		}
	...
	$(document).on( 'click', '.to-cart:not(.read_more), .basket_item_add', function(e){
		...
		if(!th.hasClass('clicked'))
		{
			...
			fill_prop.item=item;
			fill_prop.basket_props=basket_props;
			fill_prop.naborIds=naborIds;
			fill_prop.naborLid=naborLid;
			fill_prop.naborOffersCartProps=naborOffersCartProps;
			fill_prop.naborIblock=naborIblock;
			// fill_prop.naborInfo=naborInfo;
