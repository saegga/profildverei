...
$(document).ready(function (){
	...
	/* add custom class select dropdown*/
	$('.ik_select_dropdown.common_select-dd').addClass("element_6_listdropdown");
})
...
(function (window) {
					...
					for(var key in this.skuVisualParams){
						...
						if (!!TreeItems && 0 < TreeItems.length){
							for (i = 0; i < TreeItems.length; i++){
								if(TreeItems[i].getAttribute("data-onevalue") === "custom_info"){
									continue;
								}
			...
			if (!!RowItems && 0 < RowItems.length) {
				...
				if (selectMode) {
					if ($(this.obTreeRows[intNumber].LIST).parent().hasClass('ik_select')) {
						$(this.obTreeRows[intNumber].LIST).ikSelect('reset', {onShow: function(inst){console.log("show")}});
					}
	...
	/**/
	window.JCCatalogElement.prototype.VisibleCustomInfo = function()
	{
		var el = BX.findChild(this.obTree, {'tag' : "li", attribute: {"data-onevalue" : "custom_info"}}, true);

		if(el){
			el.classList.remove("missing");
			el.classList.add("custom_info");
			el.style.display = null;
			var top = el.offsetHeight;

			var tooltip = BX.create(
				{
					tag: 'div',
					text : "+30% к стоимости 60 см",
					attrs: {
						className: "tooltip_custom_size",
						style: "top:-" + top + "px",
					},
				}
			);

			var containerEl = document.getElementsByClassName('product-props');
			BX.append(tooltip, document.body);

			BX.bind(el, 'mouseenter', BX.delegate(function (e) {
				var pos = e.target.getBoundingClientRect();
				tooltip.style.display = 'block';
				tooltip.style.top = (pos.top + window.scrollY - el.offsetHeight + 5) + "px";
				tooltip.style.left = (pos.left - 20) + "px";

			}, this));
			BX.bind(el, 'mouseleave', BX.delegate(function (e) {
